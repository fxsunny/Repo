...
class DataExporter:
    ...
    @staticmethod
    def save_to_excel(data: Dict, writer) -> None:
...
class GraphVisualizer:
    ...
    def plot_full_graph(self, G: nx.DiGraph) -> None:
        node_colors = [
    ...
    def plot_ring_subgraph(
        ...
        brand_names = sorted(list(set(brands.values())))
...
class StreamlitUI:
    ...
    def render_sidebar(self, data_source: str) -> None:
...
class ReviewRingDetectionApp:
    ...
    def run(self):
        ...
        self.ui.render_sidebar(data.get("source", "generated"))
        ...
        min_shared = st.slider(
            "\U0001F527 Minimum Shared Products for Strong Connection",
            1, 5, 2
        )
        ...
        suspicious_rings = detector.detect_rings(G_proj, customer_product)
    ...
    def render_ring_details(
        ...
        with self.output:
            ...
            display(widgets.HTML(
                ...
                f"<b>Brands:</b> {', '.join(ring['brands'])}</p>"
            ))
...
class JupyterRingDetectionApp:
    ...
    def _on_min_shared_change(self, change):
        ...
        suspicious_rings = detector.detect_rings(self.G_proj, self.customer_product)
...
# Keep both UI classes (StreamlitUI and JupyterUI) in the file

# Modify the main app to use appropriate UI based on environment
